...
public class TestServiceImpl implements TestService {
    ...
    public List<TestDTO> getAllTest(){
        return testRepository.findAll().stream().peek(test -> test.setTime(test.getQuestions().size() * test.getTime()))
    ...
    public TestResultDTO submitTest(SubmitTestDTO request){
        ...
        for (QuestionResponse  response: request.getResponses()) {
            ...
            if (question.getAnswer().equalsIgnoreCase(response.getSelectedOption())) {
                correctAnswers++;
            }
        }
